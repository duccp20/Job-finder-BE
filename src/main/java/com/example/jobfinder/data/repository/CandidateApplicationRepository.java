...
public interface CandidateApplicationRepository extends JpaRepository<CandidateApplication, Long> {
    ...
    @Query("SELECT YEAR(ca.createdDate), MONTH(ca.createdDate), COUNT(ca) FROM CandidateApplication ca JOIN ca.job j WHERE j.company.id = :companyId GROUP BY YEAR(ca.createdDate), MONTH(ca.createdDate)")
    List<Object[]> countApplicationsByMonth(@Param("companyId") long companyId);

    @Query("SELECT YEAR(ca.createdDate), COUNT(ca) FROM CandidateApplication ca JOIN ca.job j WHERE j.company.id = :companyId GROUP BY YEAR(ca.createdDate)")

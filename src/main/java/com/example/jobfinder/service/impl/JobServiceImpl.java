...
//    }
    ...
    private Job checkOutOfDate(Job job) {
        ...
        if (job.getStatus().getName().equals(String.valueOf(Estatus.Active)) && job.getEndDate().before(new Date())) {
            job.setStatus(statusService.findByName(String.valueOf(Estatus.Disable)).orElseThrow(
                    () -> new ResourceNotFoundException(Collections.singletonMap("name", Estatus.Disable.toString()))));

...
public class HRServiceImpl implements HRService {
    ...
    public HRDTO findById(long id) {
        return hrMapper.toDTO(hrRepository
                .findById(id)
    ...
    public Object create(HRCreationDTO hrCreationDTO, MultipartFile fileAvatar) throws IOException {
        ...
        if (hrCreationDTO.getHrOtherInfoDTO().getCompanyDTO().getId() == null) {
            hrCreationDTO
                    .getHrOtherInfoDTO()
                    .setCompanyDTO(companyService.create(
                            hrCreationDTO.getHrOtherInfoDTO().getCompanyDTO(), null));
        } else {
            hrCreationDTO
                    .getHrOtherInfoDTO()
                    .setCompanyDTO(companyService.findById(
                            hrCreationDTO.getHrOtherInfoDTO().getCompanyDTO().getId()));
        }
    ...
    public Object updateHRInfo(HRProfileDTO hrProfileDTO, MultipartFile fileAvatar) throws IOException {
        ...
        HR hr = this.hrRepository
                .findByUsername(username)
                .orElseThrow(() -> new AccessDeniedException(Collections.singletonMap("username", username)));

        UserDTO updatedUserDTO = userService.update(hr.getUser().getId(), hrProfileDTO.getUserProfileDTO(), fileAvatar);

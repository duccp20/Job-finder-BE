...
public class CandidateApplicationServiceImpl implements CandidateApplicationService {
    ...
    public PaginationDTO findAllByCandidateId(int no, int limit) {
        ...
        return new PaginationDTO(
                applicationPages.getContent(),
                applicationPages.isFirst(),
                applicationPages.isLast(),
                applicationPages.getTotalPages(),
                applicationPages.getTotalElements(),
                applicationPages.getSize(),
    ...
    @Override
    public CandidateApplicationDTO create(CandidateApplicationDTO candidateApplicationDTO, MultipartFile file)
            throws IOException {
        ...
        if (this.existsByJobIdAndCandidateId(candidateApplicationDTO.getJobDTO().getId(), candidateDTO.getId())) {
            throw new InternalServerErrorException(
                    Collections.singletonMap("message", "You have already applied for this job!"));
        }

        // check job apply
        JobDTO jobDTO = jobService.findById(candidateApplicationDTO.getJobDTO().getId());
        candidateApplicationDTO.setJobDTO(jobDTO);
        if (!jobService.isAppliable(jobDTO)) throw new InternalServerErrorException("Job is not appliable");
        ...
        candidateApplicationDTO.setFullName(candidateDTO.getUserDTO().getLastName() + " "
                + candidateDTO.getUserDTO().getFirstName());
